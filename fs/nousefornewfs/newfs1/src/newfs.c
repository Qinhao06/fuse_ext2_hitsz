...
static struct fuse_operations operations = {
	...
	.write = newfs_read,								  	 /* 写入文件 */
	.read = newfs_write,								  	 /* 读文件 */
...
int newfs_read(const char* path, char* buf, size_t size, off_t offset,
	...
	for(int i = 0; i < NEWFS_DATA_PER_FILE; i++){
		if(offset_remain > NEWFS_BLKS_SZ(1)){
			...
			int bufcnt = 0;
			while(i < NEWFS_DATA_PER_FILE && size_remian > NEWFS_BLKS_SZ(1)){
				memcpy(buf + bufcnt * NEWFS_BLKS_SZ(1), inode->block_pointer[i] + offset_remain, NEWFS_BLKS_SZ(1) - offset_remain);
				size_remian = size_remian - NEWFS_BLKS_SZ(1) + offset_remain ;
				offset_remain = 0;
				bufcnt++;
				i++;
			}
			if(size_remian  > NEWFS_BLKS_SZ(1) - offset_remain){
				memcpy(buf + bufcnt * NEWFS_BLKS_SZ(1), inode->block_pointer[i] + offset_remain, NEWFS_BLKS_SZ(1) - offset_remain);
				size_remian -= (NEWFS_BLKS_SZ(1) - offset_remain); 
				offset_remain = 0;
				i++;
				bufcnt++;
				memcpy(buf + bufcnt * NEWFS_BLKS_SZ(1), inode->block_pointer[i] + offset_remain, size_remian);
			}
			else{
				memcpy(buf + bufcnt * NEWFS_BLKS_SZ(1), inode->block_pointer[i] + offset_remain, size_remian);
...
int newfs_write(const char* path, const char* buf, size_t size, off_t offset,
	...
	for(int i = 0; i < NEWFS_DATA_PER_FILE; i++){
		if(offset_remain > NEWFS_BLKS_SZ(1)){
			...
			int bufcnt = 0;
			while(i < NEWFS_DATA_PER_FILE && size_remian > NEWFS_BLKS_SZ(1)){
				memcpy(inode->block_pointer[i] + offset_remain, buf + bufcnt * NEWFS_BLKS_SZ(1), NEWFS_BLKS_SZ(1) - offset_remain);
				size_remian = size_remian - NEWFS_BLKS_SZ(1) + offset_remain ;
				offset_remain = 0;
				bufcnt++;
				i++;
